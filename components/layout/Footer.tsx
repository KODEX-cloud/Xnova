...
const DEFAULT_CITIES = ["Abidjan", "Cocody", "Plateau", "Marcory", "Yopougon", "Treichville", "San-Pédro"];
...
export default function Footer() {
  ...
  const columns = (() => {
    if (info.footerColumns) {
      try {
        const parsed = JSON.parse(info.footerColumns);
  ...
  const cities = (() => {
    if (info.footerCities) {
      try {
        const parsed = JSON.parse(info.footerCities);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      } catch {}
    }
  ...
  const logoText = info.logoText || "NOVA";
  const logoTagline = info.logoTagline || "Auto & Immobilier";
  const newsletterTitle = info.newsletterTitle || "Restez informé des meilleures offres";
  const newsletterSubtitle = info.newsletterSubtitle || "Recevez en avant-première nos annonces exclusives et conseils immobiliers & auto";
    ...
    <footer className="bg-gray-50 border-t border-gray-100">
      ...
      <div className="bg-gradient-to-r from-nova-red to-nova-orange">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="text-xl font-bold text-white mb-1">{newsletterTitle}</h3>
              <p className="text-white/80 text-sm">{newsletterSubtitle}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-5 group w-fit">
              {info.logoUrl ? (
                ...
                <img src={info.logoUrl} alt={logoText} className="h-9 w-auto object-contain" />
              ) : (
                <>
                  <div className="relative w-9 h-9 flex items-center justify-center">
                    ...
                    <span className="relative text-white font-black text-lg z-10">{logoText.charAt(0)}</span>
                  </div>
          ...
          {columns.map((section: { title: string; links: { label: string; href: string }[] }) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex gap-4">
              ...
              <Link href="/sitemap" className="text-gray-400 text-xs hover:text-nova-red transition-colors">Plan du site</Link>
            </div>
          </div>
        </div>
      </div>
