...
export default function ServiceDetailPage({ serviceId }: ServiceDetailPageProps) {
  ...
  // Resolve dynamic content from CMS (with fallback to hardcoded)
  const gallery = (() => {
    if (pageContent.gallery) {
      const urls = pageContent.gallery.split("\n").map(u => u.trim()).filter(Boolean);
  ...
  const features = (() => {
    if (pageContent.features) {
      const lines = pageContent.features.split("\n").map(l => l.trim()).filter(Boolean);
  ...
  // gallery and features resolved from CMS above
