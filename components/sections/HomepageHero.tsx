        ...
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-20 items-center min-h-[calc(100vh-5rem)] py-16 lg:py-24">
          ...
          <div className="order-2 lg:order-1">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.24 }}
              ...
              {(config.heroTrustBadges || []).map(({ icon: iconName, label, color }) => {
                const ICON_MAP: Record<string, typeof CheckCircle2> = { CheckCircle2, Shield, Zap };
            ...
            <div className="relative w-full max-w-sm xl:max-w-md">
              <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                ...
                <div className="h-52 bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] relative overflow-hidden">
                  ...
                  <span className="absolute top-4 left-4 px-3 py-1.5 bg-gradient-to-r from-nova-red to-nova-orange rounded-full text-white text-xs font-bold shadow-lg">{config.heroCardBadge}</span>
                  <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-black/80 to-transparent" />
                  <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between">
                    <div>
                      <p className="text-white font-black text-lg leading-none">{config.heroCardPrice}</p>
                ...
                <div className="p-5">
                  <p className="text-gray-900 font-black text-base mb-1">{config.heroCardTitle}</p>
                  <div className="flex items-center gap-1.5 text-gray-400 text-xs mb-4">
                    <MapPin className="h-3.5 w-3.5 text-nova-red flex-shrink-0" /> {config.heroCardLocation}
                  </div>
                  <div className="flex items-center gap-2 mb-5 flex-wrap">
                    {(config.heroCardSpecs || []).map((spec) => (
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-nova-red font-black text-lg">{config.heroCardPrice}</p>
                    <Link href={config.heroCardLink || "/automobile/vente"} className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-nova-red to-nova-orange text-white text-sm font-bold rounded-xl hover:shadow-lg hover:scale-105 transition-all">
              ...
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.55, duration: 0.5 }}
                ...
                {(config.heroActivity || []).map((a, i) => (
                  <div key={i} className="flex items-start gap-2 mb-2 last:mb-0">
                    <div className={`w-1.5 h-1.5 rounded-full ${i === 0 ? "bg-emerald-400" : "bg-nova-red"} mt-1.5 flex-shrink-0`} />
                    <div>
              ...
              {config.heroPopularBadge && (
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.65 }}
                  ...
                  <TrendingUp className="h-3 w-3" /> {config.heroPopularBadge}
                </motion.div>
              )}
            </div>
          </div>
