<?php
/**
 * NOVA Marketplace — Edit Promotion View
 */
?>
<div style="margin-bottom: 2rem;">
    ...
    <h1 style="font-size: 1.75rem; font-weight: 800; color: var(--slate-900);">Éditer la promotion : <?= htmlspecialchars($promotion['title']) ?></h1>
</div>

<div class="admin-card">
    <h2 class="admin-card-title">Détails de la promotion / bannière</h2>
    
    <form action="<?= BASE_URL ?>/admin/promotions/update/<?= $promotion['id'] ?>" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem; align-items: start;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" class="form-control" value="<?= htmlspecialchars($promotion['title']) ?>" required>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        <label class="form-label" for="subtitle">Sous-titre</label>
                        <input type="text" id="subtitle" name="subtitle" class="form-control" value="<?= htmlspecialchars($promotion['subtitle'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="discount">Libellé Réduction / Offre</label>
                        <input type="text" id="discount" name="discount" class="form-control" value="<?= htmlspecialchars($promotion['discount'] ?? '') ?>">
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label" for="description">Description (Optionnel)</label>
                    <textarea id="description" name="description" class="form-control" rows="4"><?= htmlspecialchars($promotion['description'] ?? '') ?></textarea>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        <label class="form-label" for="link">Adresse de Redirection (URL du lien)</label>
                        <input type="text" id="link" name="link" class="form-control" value="<?= htmlspecialchars($promotion['link'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="cta">Texte d'appel à l'action (Bouton CTA)</label>
                        <input type="text" id="cta" name="cta" class="form-control" value="<?= htmlspecialchars($promotion['cta'] ?? '') ?>">
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem; background-color: var(--slate-50); padding: 1.5rem; border-radius: var(--radius-md); border: 1px solid var(--slate-200);">
                <div class="form-group">
                    ...
                    <input type="text" id="image" name="image" class="form-control" value="<?= htmlspecialchars($promotion['image'] ?? '') ?>">
                </div>

                <div class="form-group">
                    <label class="form-label" for="badge">Badge promotionnel</label>
                    <input type="text" id="badge" name="badge" class="form-control" value="<?= htmlspecialchars($promotion['badge'] ?? '') ?>">
                </div>

                <div class="form-group">
                    <label class="form-label" for="expires_at">Date d'expiration (YYYY-MM-DD HH:MM:SS)</label>
                    <input type="text" id="expires_at" name="expires_at" class="form-control" value="<?= htmlspecialchars($promotion['expires_at'] ?? '') ?>">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                    <div class="form-group">
                        <label class="form-label" for="gradient">Style dégradé (CSS)</label>
                        <input type="text" id="gradient" name="gradient" class="form-control" value="<?= htmlspecialchars($promotion['gradient'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="bg_color">Couleur Fond (Hex)</label>
                        <input type="text" id="bg_color" name="bg_color" class="form-control" value="<?= htmlspecialchars($promotion['bg_color'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                    <div class="form-group">
                        ...
                        <input type="number" id="order" name="order" class="form-control" value="<?= $promotion['order'] ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="countdown">Compte à rebours</label>
                        <input type="text" id="countdown" name="countdown" class="form-control" value="<?= htmlspecialchars($promotion['countdown'] ?? '') ?>">
                    </div>
                </div>

                <div class="form-group" style="display: flex; align-items: center; gap: 0.5rem; margin-top: 1rem;">
                    <input type="checkbox" id="is_active" name="is_active" value="1" <?= $promotion['is_active'] ? 'checked' : '' ?> style="width: 18px; height: 18px; cursor: pointer;">
