<?php
/**
 * NOVA Marketplace — Admin Edit BlogPost View
 */
?>
<div style="margin-bottom: 2rem;">
    ...
    <h1 style="font-size: 1.75rem; font-weight: 800; color: var(--slate-900);">Modifier l'article</h1>
    <p style="color: var(--slate-500);">Modifiez l'article : <?= htmlspecialchars($post['title']) ?></p>
</div>
    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem; align-items: start;">
        ...
        <div class="admin-card">
            ...
            <div class="form-group">
                ...
                <input type="text" id="title" name="title" required value="<?= htmlspecialchars($post['title']) ?>" class="form-control">
            </div>

            <div class="form-group">
                <label class="form-label" for="excerpt">Résumé court (Excerpt)</label>
                <textarea id="excerpt" name="excerpt" rows="3" class="form-control" style="resize:vertical;"><?= htmlspecialchars($post['excerpt'] ?? '') ?></textarea>
            </div>

            <div class="form-group">
                <label class="form-label" for="content">Contenu HTML <span style="color:var(--danger)">*</span></label>
                <textarea id="content" name="content" required rows="10" class="form-control" style="resize:vertical; font-family:sans-serif;"><?= htmlspecialchars($post['content'] ?? '') ?></textarea>
            </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 2rem;">
            ...
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control">
                        <option value="DRAFT" <?= $post['status'] === 'DRAFT' ? 'selected' : '' ?>>Brouillon (DRAFT)</option>
                        <option value="PUBLISHED" <?= $post['status'] === 'PUBLISHED' ? 'selected' : '' ?>>Publié (PUBLISHED)</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="category" name="category" class="form-control">
                        ...
                        <option value="automobile" <?= $post['category'] === 'automobile' ? 'selected' : '' ?>>Automobile</option>
                        <option value="immobilier" <?= $post['category'] === 'immobilier' ? 'selected' : '' ?>>Immobilier</option>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="read_time" name="read_time" value="<?= htmlspecialchars($post['read_time']) ?>" class="form-control">
                </div>

                <div class="form-group">
                    <label class="form-label" for="author">Auteur</label>
                    <input type="text" id="author" name="author" value="<?= htmlspecialchars($post['author'] ?? '') ?>" class="form-control">
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="cover_image" name="cover_image" value="<?= htmlspecialchars($post['cover_image'] ?? '') ?>" class="form-control">
                </div>
                
                <div class="form-group">
                    <label class="form-label" for="tags">Tags (JSON Array)</label>
                    <input type="text" id="tags" name="tags" value="<?= htmlspecialchars($post['tags'] ?? '[]') ?>" class="form-control" style="font-family:monospace; font-size:0.85rem;">
                </div>
            </div>
