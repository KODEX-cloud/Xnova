...
function AnnoncesContent() {
  ...
  // CMS settings from admin/annonces-page
  const [annoncesConfig, setAnnoncesConfig] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    fetch("/api/settings?prefix=annonces.")
      ...
      .then((data: Record<string, string>) => {
        setAnnoncesConfig(data || {});
        if (!searchParams.get("kind") && data.defaultKind) setKind(data.defaultKind);
        if (!searchParams.get("sort") && data.defaultSort) setSort(data.defaultSort);
      })
      .catch(() => {});
  ...
  const load = useCallback(async (p = 1) => {
    ...
    const q = new URLSearchParams({
      kind, sort, page: String(p), limit: annoncesConfig.pageSize || "24",
      ...(city      ? { city }      : {}),
      ...(priceMin  ? { priceMin }  : {}),
      ...(priceMax  ? { priceMax }  : {}),
      ...(priceType ? { priceType } : {}),
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden lg:ml-0">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            {/* Dynamic Hero Banner (from admin/annonces-page) */}
            {annoncesConfig.showHero === "true" && annoncesConfig.heroTitle && (
              <div
                className="rounded-2xl mb-6 px-8 py-10 text-white text-center"
                style={{ background: annoncesConfig.heroBg || "linear-gradient(135deg, #E44D3A, #F97316)" }}
              >
                <h2 className="text-2xl md:text-3xl font-black mb-2">{annoncesConfig.heroTitle}</h2>
                {annoncesConfig.heroSubtitle && (
                  <p className="text-white/80 text-sm max-w-lg mx-auto">{annoncesConfig.heroSubtitle}</p>
                )}
              </div>
            )}
